...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _discord_allowed_bot_users(self) -> set:
        ...
        if raw is None:
            raw = self._gate_env("DISCORD_ALLOWED_BOTS") or self._gate_env(
    ...
    def _discord_self_message_channels(self) -> set:
        ...
        if raw is None:
            raw = self._gate_env("DISCORD_SELF_MESSAGE_CHANNELS")
        return self._discord_csv_id_set(raw)
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    auto_thread_free_response_cfg = discord_cfg.get(
        "auto_thread_free_response", platform_extra_cfg.get("auto_thread_free_response")
    )
    if auto_thread_free_response_cfg is not None:
        seeded_extra["auto_thread_free_response"] = str(auto_thread_free_response_cfg).lower()
    ...
    self_message_channels_cfg = discord_cfg.get(
        "self_message_channels", platform_extra_cfg.get("self_message_channels")
    )
    if self_message_channels_cfg is not None:
        if isinstance(self_message_channels_cfg, list):
            self_message_channels_cfg = ",".join(str(v) for v in self_message_channels_cfg)
        seeded_extra["self_message_channels"] = str(self_message_channels_cfg)
        if not _skip_env_bridge and not os.getenv("DISCORD_SELF_MESSAGE_CHANNELS"):
            os.environ["DISCORD_SELF_MESSAGE_CHANNELS"] = str(self_message_channels_cfg)
    if "reactions" in discord_cfg and not os.getenv("DISCORD_REACTIONS"):
        os.environ["DISCORD_REACTIONS"] = str(discord_cfg["reactions"]).lower()
    allow_bots_cfg = discord_cfg.get("allow_bots", platform_extra_cfg.get("allow_bots"))
    ...
    allowed_bots = discord_cfg.get(
        "allowed_bot_users",
        discord_cfg.get(
            "allowed_bots",
            platform_extra_cfg.get("allowed_bot_users", platform_extra_cfg.get("allowed_bots")),
        ),
    )
    if allowed_bots is not None:
        if isinstance(allowed_bots, list):
            allowed_bots = ",".join(str(v) for v in allowed_bots)
        seeded_extra["allowed_bot_users"] = str(allowed_bots)
        if not _skip_env_bridge and not os.getenv("DISCORD_ALLOWED_BOTS"):
